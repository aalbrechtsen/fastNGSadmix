...
if (!(length(args) %in% c(2, 4))) {
  stop(
    paste(
      "Usage:",
      "Rscript scripts/plot_single_bam_admix.R <qopt> <out_main_png>",
      "or",
      "Rscript scripts/plot_single_bam_admix.R <qopt> <boot_qopt> <out_main_png> <out_boot_png>"
    )
  )
}

qopt_file <- args[1]
boot_mode <- length(args) == 4
if (boot_mode) {
  boot_file <- args[2]
  out_main <- args[3]
  out_boot <- args[4]
